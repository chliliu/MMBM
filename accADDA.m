function [X1, Y1, info]=accADDA(m, n, W, opts)
%
...
abserr_upd1=max(max(Xdiff1));
err_upd1=max(max(Xdiff1./X1));
...
tmp=Xdiff0-Xdiff1;
err_stop=max(max( (Xdiff1.*Xdiff1)./(tmp.*X1+1e-250) ));
errs_stop=err_stop;
tol=1;
...
%while ( (err_stop > opts.tol) || (abserr_upd0 < abserr_upd1) ) && itn<opts.maxitn
   ...
   tol=max(max(max(abs(ERX))),max(max(abs(ERY))));
    ...
    tmp=Xdiff0-Xdiff1;
    err_stop=max(max( (Xdiff1.*Xdiff1)./(tmp.*X1+1e-300) ));
